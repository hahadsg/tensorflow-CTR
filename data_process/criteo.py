...
"""
This code referenced from [here](https://github.com/lambdaji/tf_repos/blob/master/deep_ctr/Feature_pipeline/get_criteo_feature.py)
...
def preprocess(datadir, outdir):
    ...
    dists.build(os.path.join(FLAGS.input_dir, 'train.txt'), continous_features)
    # pool.apply(dists.build, args=(FLAGS.input_dir + 'train.txt', continous_features,))

    dicts = CategoryDictGenerator(len(categorial_features))
    dicts.build(os.path.join(FLAGS.input_dir, 'train.txt'), categorial_features, cutoff=FLAGS.cutoff)
    ...
    # for validation.
    with open(os.path.join(FLAGS.output_dir, 'tr.libsvm'), 'w') as out_train:
        with open(os.path.join(FLAGS.output_dir, 'va.libsvm'), 'w') as out_valid:
            with open(os.path.join(FLAGS.input_dir, 'train.txt'), 'r') as f:
                for line in f:
                    features = line.rstrip('\n').split('\t')
